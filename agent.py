...
class Replaymemory: # 记忆类
    def __init__(self, n_s, n_a):
        self.n_s = n_s
        self.n_a = n_a
        self.MEMORY_SIZE = 1000
        self.BATCH_SIZE = 64    # 在1000个中随机取64个经验指导下一个决策,若不足64个，则有几个取几个
    ...
    def add_memo(self, s, a, r, done, s_):
        ...
        self.t_memo = (self.t_memo + 1) % self.MEMORY_SIZE  # 防止记忆池超载，循环填入

    def sample(self):
        if self.t_max >= self.BATCH_SIZE:
            index = random.sample(range(self.t_max), self.BATCH_SIZE)    # 当前最大的次数
        else:
            index = range(0, self.t_max)
